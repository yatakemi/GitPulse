const translations = {
    ...
    ja: {
        ...
        status_declined: "悪化",
        status_stable: "安定",
        desc_throughput: "デリバリー量を測定。計算式: [マージPR数] / [期間内の週数]。高いほどチームが多くのタスクを完了しています。",
        desc_p90: "最悪ケースのデリバリー速度。計算式: PRの90%がマージされる閾値。これを下げることで、「滞留」するPRが減ります。",
        desc_stability: "予測可能性を測定。計算式: リードタイムの標準偏差。低いほど、著者やタスクに関わらず一貫したデリバリーを示します。",
        desc_rework: "方向性の品質を測定。計算式: [変更要求またはイテレーション>1のPR] / [総PR数]。",
        desc_steps: "コード量を測定。計算式: [総追加行数] / [期間内の週数]。実装の取り組みの推移を追跡します。",
        desc_commit_density: "活動頻度を測定。計算式: [総コミット数] / [期間内の週数]。作業が細かく分割されているかを確認します。",
        desc_review_cycles: "レビューからマージまでのサイクル。PRあたりのレビュー提出の総数として計算されます（同日の複数のやり取りもカウント）。",
